...
#                    print(self.values.keys())
                    for key, value in self.values.items():
                        if isinstance(self.values[key], torch.Tensor):
                            #print("hi")
                            self.values[key] = self.values[key].cpu().data.numpy()
#                        print(type(self.values[key]))
                        if isinstance(self.values[key], list):
#                            print("ggggggggggggggg")
                            
#                            print(type(self.values[key][0]))
                            for i in range(len( self.values[key])):                      
                                if isinstance(self.values[key][i], torch.Tensor):
                            #print("hi")
                                    self.values[key][i] = self.values[key][i].cpu().data.numpy()
#                                    print("new type " +str(type(self.values[key][i])))
                                if isinstance(self.values[key][i], numpy.ndarray):
                            #print("hi")
                                    self.values[key][i] = self.values[key][i].tolist()
#                                    print("new type " +str(type(self.values[key][i])))
                    if self.compactjson:
#                        for key, value in self.values.items():
#                             if isinstance(self.values[key], torch.Tensor):
